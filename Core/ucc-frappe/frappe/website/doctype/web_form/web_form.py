...
"""
	...
	#Ranz Changes Dec 9, 2025
	def load_form_data(self, context):
		context.parents = []
		try:
			if frappe.form_dict.get("from_template"):
				import re as _re, html as _html

				def _parse_template_block(raw: str, fallback_html: str) -> tuple[dict, str]:
					if not raw:
				...
				def _bind_vars(html_in: str, values: dict) -> str:
					import re as _re, html as _html
					out = html_in or ""
					...
					for k, v in (values or {}).items():
						...
						out = _re.sub(rf"\[\[\s*{_re.escape(k)}\s*\]\]", raw, out)
						out = _re.sub(rf"\{{\s*{_re.escape(k)}\s*\}}",    raw, out)

						any_tag_pat    = _re.compile(
							rf'(<(?P<tag>[a-z0-9:-]+)[^>]*class="[^"]*\b{_re.escape(k)}\b[^"]*"[^>]*>)(.*?)(</(?P=tag)>)',
							_re.I | _re.S
						)
						any_tag_tc_pat = _re.compile(
							rf'(<(?P<tag>[a-z0-9:-]+)[^>]*class="[^"]*\btc_{_re.escape(k)}\b[^"]*"[^>]*>)(.*?)(</(?P=tag)>)',
							_re.I | _re.S
						)
						out = any_tag_pat.sub(lambda m: m.group(1) + safe + m.group(4), out)
						out = any_tag_tc_pat.sub(lambda m: m.group(1) + safe + m.group(4), out)

					for k, v in (values or {}).items():
						raw = str(v)
						tb_tpl = _re.compile(
							rf'(<tbody[^>]*data-(?:tpl|template)=["\']{_re.escape(k)}["\'][^>]*>)(.*?)(</tbody>)',
							_re.I | _re.S
						)
						tb_id  = _re.compile(
							rf'(<tbody[^>]*id=["\']{_re.escape(k)}["\'][^>]*>)(.*?)(</tbody>)',
							_re.I | _re.S
						)
						out = tb_tpl.sub(lambda m: m.group(1) + raw + m.group(3), out)
						out = tb_id.sub(lambda m: m.group(1) + raw + m.group(3), out)

					return out


				for f in (self.web_form_fields or []):
					if getattr(f, "fieldtype", None) != "HTML":
						continue
					raw = getattr(f, "template_html", None)
					if not raw:
						continue
					fm_vars, tpl_body = _parse_template_block(raw, getattr(f, "options", None) or "")
					f.options = _bind_vars(tpl_body, fm_vars)
				context.web_form_doc["web_form_fields"] = [df.as_dict(no_nulls=True) for df in self.web_form_fields]
		except Exception:
			frappe.log_error(frappe.get_traceback(), "Web Form: See Template (template_html parsing/bind)")
