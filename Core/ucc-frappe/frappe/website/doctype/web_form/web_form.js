frappe.ui.form.on("Web Form", {
    ...
    if (!frm.is_new() && frm.doc.route) {
      const route = "/" + String(frm.doc.route || "").replace(/^\/+/, "") + "/new";
      let url = (frappe.utils && frappe.utils.get_url)
        ? frappe.utils.get_url(route)
        : frappe.urllib.get_full_url(route);
      url += (url.includes("?") ? "&" : "?") + "from_template=1";

      const $a = frm.sidebar.add_user_action(__("See Template"));
      $a.off("click.see_template").on("click.see_template", (e) => {
        e.preventDefault();
        window.open(url, "_blank");
      });
      frm.add_custom_button(__('See Template'), () => window.open(url, "_blank"));
    }
