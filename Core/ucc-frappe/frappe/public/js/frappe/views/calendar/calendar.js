...
frappe.views.CalendarView = class CalendarView extends frappe.views.ListView {
    ...
    if (route.length === 3) {
        const doctype = route[1];
        const user_settings = frappe.get_user_settings(doctype)["Calendar"] || {};
        const last = user_settings.last_calendar;
        ...
        frappe
            ...
            .then((r) => {
                const customs = (r && r.message) || [];
                let target;

                if (customs.length) {
                    target = (last && customs.find(d => d.name === last) && last) || customs[0].name;
                } else if (frappe.views.calendar[doctype]) {
                    target = "default";
                }
                ...
                if (target) {
                    ...
                    frappe.msgprint(__("No calendars defined for {0}.", [__(doctype)]));
                }
            });
    ...
    return new Promise((resolve) => {
        if (calendar_name === "default") {
            frappe.call("frappe.client.get_list", {
                ...
                if (customs.length) {
                    frappe.route_flags.replace_route = true;
                    frappe.set_route("List", this.doctype, "Calendar", customs[0].name);
                    return;
                }
                Object.assign(options, frappe.views.calendar[this.doctype]);
                resolve(options);
            ...
            frappe.model.with_doc("Calendar View", calendar_name, () => {
                ...
                if (!doc) {
                    frappe.show_alert(
                        __("{0} is not a valid Calendar. Redirecting to default Calendar.", [calendar_name.bold()])
                    );
                    frappe.set_route("List", this.doctype, "Calendar", "default");
                    return;
                }
