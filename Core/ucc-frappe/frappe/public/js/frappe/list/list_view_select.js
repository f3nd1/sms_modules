...
frappe.views.ListViewSelect = class ListViewSelect {
				...
				action: () => {
					this.get_calendars().then((calendars) => {
						if (calendars && calendars.length) {
							const last = (frappe.get_user_settings(this.doctype)["Calendar"] || {}).last_calendar;
							const target = calendars.find(c => c.name === last) || calendars[0];
							frappe.set_route(target.route);
						} else if (frappe.views.calendar[this.doctype]) {
							this.set_route("calendar", "default");
						} else {
							frappe.msgprint(__("No calendars defined for {0}.", [__(this.doctype)]));
						}
					});
				},
    ...
    return frappe.call("frappe.client.get_list", {
        ...
        const has_custom = Array.isArray(result) && result.length > 0;
