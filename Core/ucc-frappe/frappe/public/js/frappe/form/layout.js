...
frappe.ui.form.Layout = class Layout {
	...
	set_tab_as_active() {
			...
			// Child table row form:
			// keep tab state local to this row form only
			if (this.grid_row_form) {
				let active_fieldname = this.grid_row_form.active_tab_fieldname;

				let active_tab = active_fieldname
					? this.tabs.find(
							(tab) =>
								tab.df.fieldname === active_fieldname && !tab.is_hidden()
					)
					: null;

				if (!active_tab) {
					active_tab = this.tabs.find((tab) => !tab.is_hidden());
				}

				active_tab && active_tab.set_active();
				return;
			}

			// Normal parent form behavior
			let frm_active_tab = this.frm?.get_active_tab?.();

			if (
				frm_active_tab &&
				this.tabs.includes(frm_active_tab) &&
				!frm_active_tab.is_hidden()
			) {
