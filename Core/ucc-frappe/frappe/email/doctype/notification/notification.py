...
"""
	...
	def send_an_email(self, doc, context):
		...
		if doc.doctype != "Communication":
			...
			# _make may return None or not return a dict in some setups; be defensive
			if isinstance(out, dict):
				communication = out.get("name")
			else:
				communication = None

			# Fallback: ensure Sales Invoice always gets a Communication row
			if doc.doctype == "Sales Invoice" and not communication:
				communication = _force_make_communication(
					reference_doc=doc,
					subject=subject,
					content=message,
					sender=sender,
...
def _force_make_communication(reference_doc, subject, content, sender, recipients, cc=None, bcc=None):
	cc = cc or []
	bcc = bcc or []
	...
	comm.reference_doctype = get_reference_doctype(reference_doc)
	comm.reference_name = get_reference_name(reference_doc)

	comm.subject = subject
	comm.content = content

	# Standard Communication fields
	comm.sender = sender or ""
	comm.recipients = "\n".join(recipients or [])
	comm.cc = "\n".join(cc)
	comm.bcc = "\n".join(bcc)
