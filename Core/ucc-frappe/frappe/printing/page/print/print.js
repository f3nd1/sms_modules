...
frappe.ui.form.PrintView = class {
	...
	get_print_html(callback) {
		...
		this._req = frappe.call({
			...
			callback: function (r) {
				if (!r.exc) {
                    $('input[data-fieldname="print_format"]').val(r.message.print_format);
                    callback(r.message);
				}
			},
		});
	...
	selected_format() {
    const doctype_default =
        this.frm && this.frm.meta && this.frm.meta.default_print_format;
    return this.print_format_selector.val() || doctype_default || "Standard";
	}
