...
@frappe.whitelist()
def mark_attendance(
	students_present, students_absent, course_schedule=None, student_group=None, date=None, stud_with_diff_status = None
):
	...
	for d in present:
		make_attendance_records(
			...
			student_group, 
			date,
			float(d.get("duration_attended", 0) or 0),
			float(d.get("expected_duration", 0) or 0),
			remarks=d["remarks"],
			minutes_late=d["minutes"]
		)
	...
	for d in absent:
		make_attendance_records(
			...
			student_group, 
			date,
			0,  # Duration attended is 0 for absent students
			float(d.get("expected_duration", 0) or 0),
			remarks=d["remarks"],
			minutes_late=d["minutes"]
		)

	for d in json.loads(stud_with_diff_status):
		make_attendance_records(
			d["student"], 
			d["student_name"], 
			d["status"], 
...
def make_attendance_records(
	...
	duration_attended=0, expected_duration=0,remarks= None,minutes_late=0
):
	...
	:param duration_attended: Duration attended in minutes
	:param expected_duration: Expected duration in minutes
	"""
	# Convert duration values to float and handle None values
	duration_attended = float(duration_attended or 0)
	...
	if existing_att:
		frappe.db.sql("Delete from `tabStudent Attendance` where name =%s",(existing_att))
		frappe.db.commit()
		# frappe.db.sql("Delete from `tabStudent Attendance` where name =%s and custom_minutes_late!=%s",(existing_att,str(minutes_late)))
		# frappe.db.sql("Delete from `tabStudent Attendance` where name =%s and custom_remark!=%s",(existing_att,remarks))
	existing_att = frappe.db.get_value(
			"Student Attendance",
			{
                "student": student,
	...
	if not existing_att:
		...
		student_attendance.custom_duration_attended = duration_attended
		student_attendance.custom_expected_duration = expected_duration
		student_attendance.custom_remark = remarks
		student_attendance.custom_minutes_late = minutes_late
		student_attendance.leave_application=frappe.get_value("Student Leave Application",{
					"student": student,
					"docstatus": 1,  
					"from_date": ["<=", date],
					"to_date": [">=", date]
				},['name']) if leave_exists else ""
...
@frappe.whitelist()
def get_students(student_group, course_schedule=None):
    ...
    # =========================================================================
    # // 🎯 THE EXEMPTION FILTER: Exclude students saved inside students_json
    # =========================================================================
    exempted_student_ids = []
    
    if student_group and course_schedule:
        # Fetch students_json directly from the specific Schedules child table row
        students_json_str = frappe.db.get_value(
            "Module Class Details Schedules Childtable",
            {
        ...
        if students_json_str:
            try:
                # Parse the JSON string string text: [{"student": "STU-001"}, {"student": "STU-002"}]
                parsed_json = json.loads(students_json_str)
                exempted_student_ids = [item.get("student") for item in parsed_json if item.get("student")]
            except (json.JSONDecodeError, TypeError):
                exempted_student_ids = []

    # Filter out students whose ID is found in the exempted list array mapping
    if exempted_student_ids:
        students = [s for s in students if s.student not in exempted_student_ids]
    # =========================================================================
	
	# Get existing attendance records for the remaining active students
    for student in students:
        attendance = frappe.db.get_value(
            ...
            ["status", "custom_duration_attended", "custom_expected_duration"]
        )
        
        if attendance:
            student.status = attendance[0]
            student.duration_attended = attendance[1] or 0
            student.expected_duration = attendance[2] or 0
        else:
            student.status = None
            student.duration_attended = 0
            student.expected_duration = 0
            
    return students

@frappe.whitelist()
def check_existing_attendance(course_schedule,student_group):
	# student_group = frappe.db.get_value("Course Schedule", course_schedule, "student_group")
	students = frappe.get_all(
        "Student Group Student",
        fields=["student", "student_name"],
	...
	for student in students:
		...
		attendance = frappe.db.get_value(
            ...
            ["status","custom_minutes_late","custom_remark","name"]
        )
		attendance_status.append({'student': student.student,'attendance': attendance})
		if attendance:
			exist_att=1
		
		active_leaves[student.student]= len(frappe.get_all(
	...
	return exist_att,attendance_status,active_leaves
@frappe.whitelist()
...
def mark_assessment_result(assessment_plan, scores):
	...
	for criteria in student_score.get("assessment_details"):
		criteria_details = frappe.db.sql("""SELECT schedule_date, from_time, to_time FROM `tabAssessment Plan Criteria` WHERE parent=%s AND assessment_criteria=%s""",(assessment_plan, criteria),  as_dict=1)
		assessment_details.append(
			{
				"assessment_criteria": criteria,
				"schedule_date": criteria_details[0].schedule_date,
				"from_time": criteria_details[0].from_time,
				"to_time": criteria_details[0].to_time,
